...
!endif
!ifndef ForgeIcon
  !error "ForgeIcon define is required"
!endif
...
!define MUI_ICON "${ForgeIcon}"
!define MUI_UNICON "${ForgeIcon}"
...
Section "Forge Code-OSS Workbench" MainSection
  ...
  WriteRegStr HKCU "${UninstallKey}" "DisplayIcon" "$INSTDIR\forge.ico"
  ...
  CreateShortcut "$SMPROGRAMS\Forge Local Agent IDE\Forge Local Agent IDE.lnk" "$INSTDIR\Forge.Installed.cmd" "" "$INSTDIR\forge.ico" 0
  CreateShortcut "$SMPROGRAMS\Forge Local Agent IDE\Uninstall Forge.lnk" "$INSTDIR\Uninstall Forge.exe"
  CreateShortcut "$DESKTOP\Forge Local Agent IDE.lnk" "$INSTDIR\Forge.Installed.cmd" "" "$INSTDIR\forge.ico" 0
SectionEnd
